...
module Rack
  ...
  class Proxy
    ...
    def perform_request(env)
      ...
      url = @backend || source_request

      # Create a streaming response (the actual network communication is deferred, a.k.a. streamed)
      if @streaming
        target_response = HttpStreamingResponse.new(target_request, url.host, url.port)
        target_response.use_ssl = url.scheme == "https"
      else
        target_response = Net::HTTP.start(url.host, url.port) do |http|
